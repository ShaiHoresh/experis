...
UNIT(simple_create)
    HashMap* map;
    map = HashMapCreate(100, SimpleIntHash, CompareInt);
...
UNIT(TestRemoving)
    ...
    *d = 3;
    ...
    map = HashMapCreate(10, SimpleIntHash, CompareInt);
    HashMapInsert(map, a, b);
    
    ret = HashMapRemove(map, a, (void**)&c, (void**)&d);
    
    ASSERT_THAT(MAP_SUCCESS == ret && *c == *a);
...
UNIT(TestBigHash)
    ...
    map = HashMapCreate((unsigned int)(SIZE * 1.3), SimpleIntHash, CompareInt);
    ...
    for(i = 0; i < SIZE; ++i)
    {
        ...
        key[i] = &zxcv[i];
        value[i] = &asdf[i];
        ret = HashMapInsert(map, key[i], value[i]);
    }
    ASSERT_THAT(MAP_SUCCESS == ret && i == SIZE);
