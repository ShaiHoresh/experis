...
/*	int i;
	...
	int arr[] = {30, 12, 23, 25, 35, 54, 45, 56};
	
	vector = VectorCreate(10, 2);
	vec2 = VectorCreate(10, 2);
	vec3 = VectorCreate(10, 2);
	vec4 = VectorCreate(10, 2);
	
	for(i = 0; i < 8; ++i)
	{
	    VectorAdd(vector, arr[i]);
	    VectorAdd(vec3, arr[i]);
	    VectorAdd(vec2, arr[i]);
	    VectorAdd(vec4, arr[i]);
	}
	...
	BubbleSort(vec2);
    QuickSortRec(vec3);
    QuickSortIterative(vec4);
    
    printf("array after Shake sort:             ");
	VectorPrint(vector);
    printf("array after Bubble sort:            ");
	VectorPrint(vec2);
    printf("array after recursive Quick sort:   ");
	VectorPrint(vec3);
    printf("array after Iterative Quick sort:   ");
	VectorPrint(vec4);	
