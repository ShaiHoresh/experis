...
*/
static void CountAppereance(Vector* _vec, int* bArray, int numOfItems);
...
ADTErr CountingSort(Vector* _vec, int _maxValue)
{
    ...
    CountAppereance(_vec, indexArray, numOfItems);
...
ADTErr RadixSort(Vector* _vec, int _nDigits)
{
    ...
    indexArray = calloc(10, sizeof(int)); /* array C - for appereance counting*/
    if(NULL == indexArray)
    {
        return ERR_ALLOCATION_FAILED;
    }
...
static void CountAppereance(Vector* _vec, int* _indexArray, int _numOfItems)
{
    int i;
    int value;
    ...
    for(i = 0; i < _numOfItems; ++i)
    {
        ...
        ++_indexArray[value];       /*bArray[i] = value;*/
    }
}
...
static void PutInPlace(Vector* _vec, int* bArray, int* _indexArray, int _numOfItems)
{
    int i, value, newIndex;
    
    for(i = _numOfItems - 1; i >= 0; --i)
    {
        VectorGet(_vec, i, &value);     /*value = bArray[i];*/
