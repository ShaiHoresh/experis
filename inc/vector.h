 ...
 * @return void
 */
void VectorDestroy(Vector** _vector, VectorElementAction _destroy);
 ...
 * @retval ERR_NOT_INITIALIZED of the vector is NULL
 * @retval ERR_OVERFLOW if voctor is full and _blockSize is zero
 ...
 * @retval ERR_OK on success 
 * @retval ERR_REALLOCATION_FAILED if had to reallocate, but failed.
 * @retval ERR_UNDERFLOW if the vector is already empty .
 ...
 * @retval ERR_OK on success.
 * @retval ERR_NOT_INITIALIZED of the vector is NULL.
 * @retval ERR_WRONG_INDEX if the idnex is out of range.
