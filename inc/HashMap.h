...
typedef enum MapResult {
	...
	MAP_PROTECTION_ERROR            /**< Mutex failure              */
} MapResult;
...
typedef int (*CallBack)(void** _existValue, void* _updateValue);
 ...
 */
MapResult HashMapRehash(HashMap *_map, unsigned int newCapacity);
...
/**
 * @brief Do an action on value found by key
 * complexty(n+k)
 * @param[in] _map - Hash map to use, must be initialized
 * @param[in] _searchKey - key to serve as index for search
 * @param[in] _callBack - function to make an action on the value
 * @param[out] _pValue - pointer to variable that will get the value assoiciated with the search key.
 * @return Success indicator
 * @retval  MAP_SUCCESS	on success
 * @retval  MAP_KEY_NULL_ERROR
 ...
 * @warning complexity can be O(n+k)
 */
